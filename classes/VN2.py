...
class ISGN(nn.Module):
    ...
    def get_input_info(self, x, edges, note_locations, start_index):
        ...
        return beat_numbers, measure_numbers, num_notes, note_out

    def get_latent_from_info(self, y, edges, start_index, beat_numbers, measure_numbers, num_notes, note_out):
        ...
        performance_measure_nodes = self.make_higher_node(perform_style_graphed, self.performance_measure_attention,
                                                          beat_numbers,
                                                          measure_numbers, start_index, lower_is_note=True)
        perform_style_encoded, _ = self.performance_encoder(performance_measure_nodes)
        return self.performance_final_attention(perform_style_encoded)

    def encode(self, x, y, edges, note_locations, start_index):
        beat_numbers, measure_numbers, num_notes, note_out = self.get_input_info(x, edges, note_locations, start_index)
        return self.get_latent_from_info(y, edges, start_index, beat_numbers, measure_numbers, num_notes, note_out)


    def decode(self, perform_z, x, edges, beat_numbers, measure_numbers, num_notes, note_out, start_index):
        ...
        if self.is_baseline:
            ...
            out_with_result = torch.cat(
                (note_out, perform_z_batched, tempo_info_in_note, initial_beat_hidden, initial_output, initial_margin),
                2)
            ...
            for i in range(self.num_sequence_iteration):
                ...
                out_beat_spanned = self.span_beat_to_note_num(out_beat_rnn_result, measure_numbers, num_notes,
                                                              start_index)
            ...
            for i in range(self.num_sequence_iteration):
                ...
                out_in_beat = self.make_higher_node(initial_out, self.final_beat_attention, beat_numbers,
                                                    beat_numbers, start_index, lower_is_note=True)
                out_beat_cat = torch.cat(
                    (out_in_beat, margin_in_beat, beat_qpm_primo, beat_tempo_primo, beat_tempo_vector), 2)
    ...
    def forward(self, x, y, edges, note_locations, start_index, initial_z=False, return_z=False):
        beat_numbers, measure_numbers, num_notes, note_out = self.get_input_info(x, edges, note_locations, start_index)
        perform_z = self.get_latent_from_info(y, edges, start_index, beat_numbers, measure_numbers, num_notes, note_out)
        return self.decode(perform_z, x, edges, beat_numbers, measure_numbers, num_notes, note_out, start_index)

...
virtuoso_path = os.path.join(get_root_folder(), 'virtuosoNet','')
...
def load_stat_file(
    ...
    with open(virtuoso_path + data_name + "_stat.dat", "rb") as f:
...
def load_params(model_type):
    return param.load_parameters(virtuoso_path + model_type + "_param")
...
def load_model(model_type, device=virtuoso_settings.DEVICE):
    ...
    checkpoint = torch.load(virtuoso_path + model_type + '_best.pth.tar', map_location=torch.device(device))
...
def load_file_and_generate_performance(
    ...
    test_x, xml_notes, xml_doc, edges, note_locations = xml_matching.read_xml_to_array(
        virtuoso_path + path_name, means, stds, start_tempo, composer, vel_pair
    )
...
def load_file_and_encode_style(
    ...
    absolute_path = virtuoso_path + path
